...
<head>
    ...
    <title>{{ __('Assistants') }} - {{ config('app.name') }}</title>
...
<body class="bg-gray-100 min-h-screen">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                <div class="flex items-center gap-6">
                    <a href="/admin/dashboard" class="text-xl font-bold text-blue-600">{{ config('app.name') }}</a>
                    <div class="hidden md:flex items-center gap-4">
                        <a href="/admin/appointments" class="text-gray-600 hover:text-gray-900">{{ __('Appointments') }}</a>
                        <a href="/admin/queue" class="text-gray-600 hover:text-gray-900">{{ __('Queue') }}</a>
                        <a href="/admin/staff" class="text-gray-600 hover:text-gray-900">{{ __('Staff') }}</a>
                        <a href="/admin/reports" class="text-gray-600 hover:text-gray-900">{{ __('Reports') }}</a>
                        <a href="/admin/settings" class="text-gray-600 hover:text-gray-900">{{ __('Settings') }}</a>
                        <a href="/admin/assistants" class="text-blue-600 font-medium">{{ __('Assistants') }}</a>
                    </div>
                </div>
                <div class="flex items-center gap-4">
                    <a href="/admin/profile" class="flex items-center gap-2 text-gray-600 hover:text-gray-900">
                        @if(auth()->user()->avatar)
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ __('Assistants') }}</h1>
                <p class="text-gray-600 mt-1">{{ __('Manage assistants and their permissions') }}</p>
            </div>
            <button onclick="openAssistantModal()" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 flex items-center gap-2">
    ...
    <div id="assistantModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <h3 id="modalTitle" class="text-lg font-semibold text-gray-900 mb-4">{{ __('Add New Assistant') }}</h3>
    ...
    <script>
                ...
                if (data.data && data.data.length > 0) {
                                ...
                                <div class="flex items-center gap-4">
                                    <div class="flex flex-wrap gap-1 max-w-md">
                                        ${(assistant.permissions || []).map(p => `
                                            <span class="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">${getPermissionLabel(p)}</span>
                                        `).join('')}
                                    </div>
                                    <div class="flex gap-2">
                                        <button onclick="editAssistant(${assistant.id})" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg" title="{{ __('Edit') }}">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                            </svg>
                                        </button>
                                        <button onclick="deleteAssistant(${assistant.id})" class="p-2 text-red-600 hover:bg-red-50 rounded-lg" title="{{ __('Delete') }}">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    container.innerHTML = `
                        <div class="p-12 text-center">
                            ...
                            <h3 class="mt-4 text-lg font-medium text-gray-900">{{ __('No assistants yet') }}</h3>
                            <p class="mt-2 text-gray-500">{{ __('Add your first assistant to help manage the system') }}</p>
                            <button onclick="openAssistantModal()" class="mt-4 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                                {{ __('Add Assistant') }}
                            </button>
                        </div>
                    `;
        ...
        function selectPermission(key, name) {
            ...
            tag.innerHTML = `
                ${name}
                <button type="button" onclick="removePermission('${key}')" class="text-blue-600 hover:text-blue-800 focus:outline-none">
        ...
        async function editAssistant(id) {
            try {
                ...
                if (data.success) {
                    ...
                    // Add existing permissions as tags
